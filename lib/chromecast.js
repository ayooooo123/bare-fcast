...
/**
 * Ceiling on both the receive buffer and any single declared message length.
 *
 * The CASTV2 frame header is a peer-controlled 32-bit length, so this is what
 * stops a receiver from asking us to hold 4 GiB.
 */
...
export function encodeFieldVarint(fieldNumber, value) {
  const tag = (fieldNumber << 3) | 0
  return Buffer.concat([encodeVarint(tag), encodeVarint(value)])
}

export function encodeFieldBytes(fieldNumber, data) {
  const tag = (fieldNumber << 3) | 2
  return Buffer.concat([encodeVarint(tag), encodeVarint(data.length), data])
}

export function encodeFieldString(fieldNumber, value) {
  const data = Buffer.from(value, 'utf8')
  return encodeFieldBytes(fieldNumber, data)
}

/**
 * Encode a CastMessage protobuf body, without the length prefix.
 *
 * This is the inverse of `decodeCastBody`.
 */
export function encodeCastBody({ sourceId, destinationId, namespace, payloadUtf8, payloadBinary }) {
  const parts = [
...
/**
 * Encode a CastMessage as it goes on the wire: a 32-bit big-endian body length
 * followed by the protobuf body.
 *
 * This is the inverse of `decodeCastMessage`. If you have already stripped the
 * length prefix — as the socket read loop does — use `decodeCastBody` instead.
 */
export function encodeCastMessage(message) {
  const body = encodeCastBody(message)
...
/**
 * Decode a CastMessage protobuf body that has no length prefix.
 *
 * Unknown fields and unsupported wire types stop the parse rather than throwing:
 * a receiver is free to send fields this library does not model.
 */
export function decodeCastBody(buffer) {
  let offset = 0
  const fields = {}
...
/**
 * Decode a length-prefixed CastMessage, the inverse of `encodeCastMessage`.
 *
 * Returns null if the buffer does not hold one complete framed message. The
 * declared length is checked against what is present before the body is read.
 */
export function decodeCastMessage(buffer) {
  ...
  if (length > MAX_BUFFER_SIZE) {
    throw new Error(`CastMessage length out of range: ${length}`)
  }
  if (buffer.length < 4 + length) return null

  return decodeCastBody(buffer.slice(4, 4 + length))
}
...
export class ChromecastDevice extends EventEmitter {
  ...
  async connect(timeout = 5000) {
    ...
    this._connectPromise = new Promise((resolve, reject) => {
      ...
      let socket
      try {
        // Chromecast receivers present a self-signed certificate, so peer
        // verification has to be off or every handshake fails. bare-tls 2.x had
        // no verification at all and this was implicit; 3.x verifies against a CA
        // bundle by default, so the opt-out is now explicit and deliberate.
        //
        // Consequence: a cast target is authenticated by nothing. Discovery is
        // unauthenticated mDNS on the local network anyway, so treat the receiver
        // as untrusted and do not hand it anything you would not put in a log.
        socket = tls.createConnection({
          port: this.deviceInfo.port || CHROMECAST_PORT,
  ...
  _handleAppData(data) {
    ...
    while (this._buffer.length >= 4) {
      ...
      const message = decodeCastBody(payload)
